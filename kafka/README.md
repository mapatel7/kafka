...
* Requires at least version 3.0 of helm to support dependency management in Chart.yaml
...
```
$ git clone the repository in your workspace. The directory will be named 'kafka'.
...
$ helm install my-kafka -n kafka ./kafka
```

This chart includes a ZooKeeper chart as a dependency to the Kafka
cluster in its `Chart.yaml` as of version 3.0 of helm. The chart can be customized using the
...
```bash
$ helm install my-kafka -f values.yaml ./kafka
```
...
```
This is present as 'test-client-pod.yaml' in the directory. You can apply it as below:

` kubectl apply -f kafka/test-client-pod.yaml `

Please note that this creates the pod in the namespace 'kafka' as mentioned in the yaml file. You can adjust it accordingly.
